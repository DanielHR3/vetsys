...
from models.cita import editar_cita, eliminar_cita, guardar_cita, obtener_citas
...
def ventana_citas():
    ...
    def guardar_o_actualizar():
        ...
        if cita_editando["id"]:
            ...
            messagebox.showinfo(
                "Cita actualizada", "La cita fue modificada correctamente."
            )
            ...
            messagebox.showinfo(
                "Cita guardada", "La cita fue registrada correctamente."
            )
    ...
    def eliminar():
        ...
        if not seleccionado:
            messagebox.showwarning(
                "Selecciona una cita", "Selecciona una cita para eliminar."
            )
            return
        datos = tabla.item(seleccionado)["values"]
        confirm = messagebox.askyesno(
            "Eliminar", f"¿Eliminar la cita de {datos[1]} el {datos[2]}?"
        )
        if confirm:
    ...
    def enviar_email():
        ...
        if enviar_correo(correo, paciente, fecha, hora, motivo, archivo):
            messagebox.showinfo(
                "Correo enviado", "Confirmación enviada con archivo .ics."
            )
        else:
    ...
    ttk.Style().configure(
        "TButton", padding=6, relief="flat", background="#1976d2", foreground="black"
    )
    ...
    tk.Label(form_frame, text="Paciente", bg="#e8f0fe").grid(
        row=0, column=0, padx=10, pady=5, sticky="e"
    )
    tk.Label(form_frame, text="Fecha", bg="#e8f0fe").grid(
        row=0, column=2, padx=10, pady=5, sticky="e"
    )
    tk.Label(form_frame, text="Hora", bg="#e8f0fe").grid(
        row=1, column=0, padx=10, pady=5, sticky="e"
    )
    tk.Label(form_frame, text="Motivo", bg="#e8f0fe").grid(
        row=1, column=2, padx=10, pady=5, sticky="e"
    )
    tk.Label(form_frame, text="Correo", bg="#e8f0fe").grid(
    ...
    btn_guardar = tk.Button(
        btn_frame, text="Guardar Cita", width=15, command=guardar_o_actualizar
    )
    btn_guardar.grid(row=0, column=0, padx=5)

    tk.Button(btn_frame, text="Eliminar", width=15, command=eliminar).grid(
    ...
    tabla = ttk.Treeview(
        win,
        columns=("ID", "Paciente", "Fecha", "Hora", "Motivo"),
        show="headings",
        height=10,
    )
